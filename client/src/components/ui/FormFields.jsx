...
export function Field({ label, htmlFor, required, error, children, hint }) {
  ...
  const [filled, setFilled] = useState(
    Boolean(children?.props?.defaultValue || children?.props?.value),
  );
  const [length, setLength] = useState(
    (children?.props?.defaultValue || "").length,
  );
    ...
    <div>
        ...
        <motion.label
          ...
          variants={{
            up: {
              y: -21,
              scale: 0.82,
              color: error ? "var(--color-danger)" : "#0b7c7c",
            },
      ...
      <div className="flex items-start justify-between mt-1 gap-3">
        ...
        {isTextarea && maxLength && (
          <span
            className={`text-xs shrink-0 pt-0.5 ${length > maxLength * 0.9 ? "text-warning" : "text-muted"}`}
          >
            {length}/{maxLength}
          </span>
        )}
